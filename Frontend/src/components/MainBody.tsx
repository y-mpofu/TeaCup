...
class NewsApiService {
   ...
   */
  async fetchNewsByCategory(category: string, maxArticles: number = 5): Promise<NewsArticle[]> {
    try {
      console.log(`Fetching ${category} news from backend...`)
      
      const response = await fetch(`${this.baseUrl}/news/${category}?max_articles=${maxArticles}`)
      ...
      if (data.success) {
        console.log(`Successfully fetched ${data.articles.length} articles for ${category}`)
...
export default function MainBody({ onPlayStory }: MainBodyProps) {
  ...
  const loadNewsData = async () => {
    try {
      ...
      const categories = ['politics', 'sports', 'health', 'business', 'technology', 'local-trends']
    ...
    return (
      <div className="main-body-container">
        <div className="loading-container" style={{ 
          ...
          <Coffee size={48} className="loading-icon" style={{ marginBottom: '1rem' }} />
